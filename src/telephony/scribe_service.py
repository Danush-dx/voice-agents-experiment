...
class ScribeService:
    ...
    async def connect(self):
        ...
        try:
            logger.info(f"ScribeService: Connecting with model: {config.ELEVENLABS_STT_MODEL}")
            ...
            logger.info("ScribeService: Connected")

        except Exception as e:
            logger.error(f"ScribeService connection error: {e}")
            self.is_connected = False
            raise
    ...
    def _on_partial(self, event):
        ...
        try:
            ...
            logger.error(f"Error processing partial: {e}")
    ...
    def _on_committed(self, event):
        ...
        try:
            ...
            if text:
                ...
                logger.debug(f"Scribe Committed: {text}")
        except Exception as e:
            logger.error(f"Error processing committed: {e}")
    ...
    def _on_error(self, event):
        logger.error(f"Scribe Error Event: {event}")

    def _on_close(self, event=None):
        logger.info(f"Scribe connection closed. Event: {event}")
        self.is_connected = False
    ...
    async def send_audio(self, pcm_bytes: bytes):
        ...
        try:
            ...
            logger.error(f"ScribeService send error: {e}")
    ...
    def clear_buffer(self):
        """Discard all buffered text (echo cleanup)."""
        self.committed_text = []
        self.partial_text = ""
        logger.debug("ScribeService: Buffer cleared")
    ...
    async def close(self):
        ...
        if self.connection:
            try:
                ...
                logger.debug(f"ScribeService close: {e}")
        self.is_connected = False
