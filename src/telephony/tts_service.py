...
class TTSService:
    ...
    def __init__(self):
        ...
        logger.info(f"TTSService: Initializing Cartesia with model: {config.CARTESIA_TTS_MODEL}")
        self.client = Cartesia(api_key=api_key)
        self.model_id = config.CARTESIA_TTS_MODEL
        self.voice_id = config.CARTESIA_VOICE_ID
        logger.info(f"TTSService: Initialized with model={self.model_id}, voice={self.voice_id}")
    ...
    def synthesize(self, text: str) -> bytes:
        ...
        if not text or not text.strip():
            logger.warning("TTSService: Empty text, returning no audio")
            return b""

        try:
            logger.debug(f"TTSService: Synthesizing: '{text}'")
            ...
            logger.debug(f"TTSService: Generated {len(audio_data)} bytes of audio")
            return audio_data

        except Exception as e:
            logger.error(f"TTSService error: {e}", exc_info=True)
            return b""
    ...
    def synthesize_stream(self, text: str, language: str = "en"):
        ...
        try:
            logger.debug(f"TTSService: Streaming synthesis for: '{text}' (Lang: {language})")
            ...
            logger.debug(f"TTSService: Streaming complete ({chunk_count} chunks)")

        except Exception as e:
            logger.error(f"TTSService error during streaming: {e}", exc_info=True)
